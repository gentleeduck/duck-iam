...
export const queryKeys = {
	...
	comments: (postId?: string) =>
		postId ? (["comments", postId] as const) : (["comments"] as const),
	iam: ["iam"] as const,
};
...
export function usePost(id: string) {
	return useQuery({
		...
		queryFn: () => mockBackend.getPost(id),
		enabled: Boolean(id),
	});
}
...
export function useComments(postId?: string) {
	return useQuery({
		...
		queryFn: () => mockBackend.listComments(postId),
	});
}
...
export function useCreatePost() {
	...
	return useMutation({
		mutationFn: (input: Omit<Post, "id" | "createdAt">) =>
			mockBackend.createPost(input),
...
export function useCreateComment() {
	const qc = useQueryClient();
	return useMutation({
		mutationFn: (input: Parameters<typeof mockBackend.createComment>[0]) =>
			mockBackend.createComment(input),
		onSuccess: (c) => {
