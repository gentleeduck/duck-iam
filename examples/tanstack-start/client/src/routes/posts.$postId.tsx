...
function PostDetailPage() {
	...
	if (isLoading)
		return <main className="page-wrap px-4 pt-14 text-xs">loading…</main>;
	if (!post)
		return <main className="page-wrap px-4 pt-14 text-xs">not found</main>;
	...
	return (
		<main className="page-wrap flex flex-col gap-4 px-4 pb-8 pt-14">
			...
			<Authorize
				action="read"
				fallback={
					<p className="text-xs italic text-[var(--sea-ink-soft)]">
						hidden by policy.
					</p>
				}
				resource={resource}
			>
				<article>
					...
					<p className="text-xs text-[var(--sea-ink-soft)]">
						{post.workspaceId} · {post.published ? "published" : "draft"} ·
						owner {post.ownerId}
					</p>
			...
			<section className="rounded-xl border border-[var(--line)] p-3">
				<header className="mb-2 flex items-center justify-between">
					...
					<span className="text-[10px] text-[var(--sea-ink-soft)]">
						live trace for current subject
					</span>
				</header>
				...
				{explain.data && (
					<div className="text-xs">
						...
						<details>
							<summary className="cursor-pointer text-[var(--sea-ink-soft)]">
								policies ({explain.data.policies.length})
							</summary>
			...
			<section>
				...
				<ul className="mt-2 flex flex-col gap-1">
					{comments.map((c) => (
						<li
							className="flex items-start justify-between gap-2 rounded border border-[var(--line)] p-2 text-xs"
							key={c.id}
						>
							<div>
								<p>{c.body}</p>
							...
							<Authorize
								...
								resource={{
									type: "comment",
									id: c.id,
									attributes: { ownerId: c.ownerId, postId: c.postId },
								}}
							>
								<button
									className="rounded border border-rose-700 px-2 py-0.5 text-[10px] text-rose-500"
