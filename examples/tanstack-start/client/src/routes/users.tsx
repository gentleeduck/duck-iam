		...
		<main className="page-wrap flex flex-col gap-4 px-4 pb-8 pt-14">
			<header>
				...
				<p className="text-xs text-[var(--sea-ink-soft)]">
					RBAC assignments via <code>engine.admin.assignRole</code>. Switch user
					in the bar to act as that subject.
				</p>
			</header>
			...
			<Authorize
				action="read"
				resource={{ type: "user" }}
				fallback={<p className="text-xs italic">no permission to list users</p>}
			>
			...
			{selected && (
				<Authorize
					action="update"
					resource={{ type: "user", id: selected.id }}
					fallback={
						<p className="text-xs italic">no permission to manage roles</p>
					}
				>
					<section className="flex flex-col gap-3 rounded-xl border border-[var(--line)] p-3 text-xs">
						<header className="flex items-center justify-between">
							...
							<span className="text-[10px] text-[var(--sea-ink-soft)]">
								{assignments.length} roles defined
							</span>
						</header>
						...
						<p className="text-[10px] text-[var(--sea-ink-soft)]">
							Scope defaults to the user's workspace (
							<code>{selected.workspaceId}</code>). Use the IAM devtools page
							for cross-scope debugging.
						</p>
					</section>
				</Authorize>
			)}
