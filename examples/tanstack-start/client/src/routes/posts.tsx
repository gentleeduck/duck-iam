...
async function gateAction(
	...
	const allowed =
		typeof decision === "boolean"
			? decision
			: Boolean((decision as { allowed?: boolean }).allowed);
	if (allowed) run();
			...
			<header className="mb-4 flex items-center justify-between">
				<div>
					...
					<p className="text-xs text-[var(--sea-ink-soft)]">
						Every button below is gated by <code>engine.can()</code>. Switch
						user in the bar above to see policies fire.
					</p>
				</div>
				{me && (
					<Authorize
						...
						resource={{
							type: "post",
							attributes: { workspaceId: me.workspaceId },
						}}
					>
						<form
				...
				{posts.map((post) => (
					<PostRow
						...
						onPublish={() =>
							updatePost.mutate({
								id: post.id,
								patch: { published: !post.published },
							})
						}
						post={post}
					/>
		...
		<li className="flex items-start justify-between gap-4 rounded-xl border border-[var(--line)] p-3">
			...
			<div className="flex gap-2">
				<button
					...
					onClick={() =>
						me && gateAction(me.id, "publish", resource, onPublish)
					}
					type="button"
				>
