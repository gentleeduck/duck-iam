/**
 * The IAM vocabulary, roles and policies - everything that describes the
 * model rather than where it is stored.
 *
 * Split out of `./access` so it can be imported from the browser. That module
 * builds the Drizzle adapter and therefore pulls in `postgres`, which cannot
 * be bundled into a client component; the devtools panel needs the same
 * actions, resources and role definitions to build an engine over the HTTP
 * adapter instead. `./access` re-exports everything here, so server code that
 * already imports from it keeps working unchanged.
 */
import { createIam } from '@gentleduck/iam'

// ── Typed context for ABAC policies ────────────────────────────────

export interface DocDuckContext {
  subject: {
    id: string
