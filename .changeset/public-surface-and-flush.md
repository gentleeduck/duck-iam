...
**`Identities` is exported under its own name.** `@gentleduck/auth/core` exported it as `Identity`, so the namespace read as `Identities` in every declaration, doc comment, internal signature and error message, and as `Identity` at the import site. Both names are exported now; `Identity` is `@deprecated` and goes away in 6.0.
