...
${createIdx} ${prefix}sessions_absolute_expires ON ${prefix}sessions(absolute_expires_at);`
  ...
  // Unlike the three above, this table has no bridge row type and no writer in
  // the package: it ships as the schema the audit stream is persisted into.
  // Emitting it is still the CLI's job - a `migrate` that produced every table
  // except the audit log left consumers to hand-write the one that matters most.
  const events = `CREATE TABLE IF NOT EXISTS ${prefix}events (
  id ${t.id} PRIMARY KEY NOT NULL,
  identity_id ${t.id},
  session_id ${t.id},
  tenant_id ${t.shortText},
  event ${t.shortText} NOT NULL,
  method ${t.shortText},
  ip ${t.shortText},
  user_agent ${t.text},
  actor_id ${t.shortText},
  metadata ${t.text},
  created_at ${t.big} NOT NULL
);
${createIdx} ${prefix}events_identity_created ON ${prefix}events(identity_id, created_at);
${createIdx} ${prefix}events_tenant_created ON ${prefix}events(tenant_id, created_at);
${createIdx} ${prefix}events_actor_created ON ${prefix}events(actor_id, created_at);
${createIdx} ${prefix}events_created ON ${prefix}events(created_at);`
...
-- Columns mirror AuthSqlBridge.{IIdentityRow,ICredentialRow,ISessionRow} plus the
-- audit log the event stream is written into; bigints are ms-since-epoch; JSON
-- blobs are stored as text for cross-dialect parity.
--
...
-- events.actor_id is the operator for an audited action, where identity_id is
-- its subject; the two differ when someone acts on another account.
--
