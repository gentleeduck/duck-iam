...
suite('E2E AuthEngine on real Postgres + Redis', () => {
  ...
  /**
   * The CSRF token only ever reaches a client on a Set-Cookie intent, so read it the way the browser
   * would rather than from a return value. By the transport's own name, not a literal: this suite runs
   * `{ secure: false }` over plain http, where the companion drops its `__Host-` prefix because the
   * prefix requires Secure and a browser would discard the cookie rather than keep it.
   */
  function csrfTokenFrom(intents: Array<{ type: string; name?: string; value?: string }>): string | undefined {
    return intents.find((i) => i.type === 'setCookie' && i.name === transport.csrfCookieName)?.value
  }

  const transport = new CookieTransport({ name: 'duck-sid', secure: false })
  ...
  it('signs in, resolves the session, and signs out again', async () => {
    ...
    // The address is folded on the way in, so one spelling reaches the row on every dialect. `e2ePrefix`
    // mixes case, which is what makes this worth asserting against a real Postgres rather than a fake.
    expect(resolved?.identity?.profile.email).toBe(email.toLowerCase())

    // And the fold is what a sign-in leans on: a different capitalisation of the same address is the same
    // account, not a missing one.
    const recased = await auth.flows.signIn({
      input: { email: email.toUpperCase(), password: 'correct-horse-battery' },
      providerId: 'password',
    })
    expect(recased.session?.identityId).toBe(session?.identityId)

    await auth.flows.signOut(sid)
    await expect(auth.resolveSession({ headers: new Headers({ cookie: `duck-sid=${sid}` }) })).rejects.toMatchObject({
  ...
  it('credential-change revokes every other session for the identity', async () => {
    ...
    await expect(
      auth.resolveSession({ headers: new Headers({ cookie: `duck-sid=${first.sid}` }) }),
    ).rejects.toMatchObject({ code: 'AUTH_SESSION_REVOKED' })
    await expect(
      auth.resolveSession({ headers: new Headers({ cookie: `duck-sid=${second.sid}` }) }),
    ).rejects.toMatchObject({ code: 'AUTH_SESSION_REVOKED' })
  })
