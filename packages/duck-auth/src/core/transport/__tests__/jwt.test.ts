...
describe('AuthJwtTransport', () => {
  ...
  describe('verify failure paths', () => {
    ...
    it('returns null when issuer does not match', async () => {
      const t1 = new AuthJwtTransport(baseCfg)
      const t2 = new AuthJwtTransport({ ...baseCfg, issuer: 'https://different.example.com' })
      const token = (
  ...
  describe('revoke', () => {
    it('returns clearCookie intent when refresh enabled', () => {
      const t = new AuthJwtTransport({ ...baseCfg, refresh: { cookieName: '__Host-rt' } })
