...
/** How far ahead of the verifier an `iat` may sit and still be two clocks disagreeing. Not
 *  `clockSkewSec`, which can be zero: this one cannot, or a clock a second fast refuses every token. */
const JWT_FORWARD_SKEW_SEC = 60

/** The algorithms this transport signs and verifies; {@link JwtTransport.IJwtAlg} is derived from it. */
...
export namespace JwtTransport {
  export interface Cfg {
    ...
    freshnessMs?: number
    /** Seconds of leeway on `exp` and `abs`, for instances that disagree on the time. Default 0.
     *  WARN: it widens the window an expired token is accepted in. The forward bound on `iat` allows a
     *  fixed 60s on top of this. */
    clockSkewSec?: number
    ...
    refresh?: {
      ...
      ttlMs?: number
      /** Default `'strict'`; `'lax'` for a refresh endpoint reached by a top-level navigation.
       *  WARN: `'none'` sends this cookie, which carries the plaintext SID, on every cross-site
       *  request. An SPA on a different origin to its API needs it; nothing else does. */
  ...
  export interface Payload {
    ...
    frsh?: number
    /** `absoluteExpiresAt` in epoch seconds: the ceiling no renewal moves. Optional, so a token minted
     *  before the claim existed still verifies — absent, `verify` falls back to `exp`. */
    abs?: number
...
/** The floor every key must clear. The constructor and `rotateSignKey` install into the same two
 *  fields, and only the constructor used to check. */
function assertKeyMaterial(k: { kid: string; key: string }, field: string): void {
...
export class JwtTransport implements Transport.ITransport {
  ...
  private readonly _refreshSameSite: 'strict' | 'lax' | 'none'
  private readonly _clockSkewSec: number
  /** Read by `strict()`, which enforces the production floor. A boolean, never the key. Computed on
   *  read: frozen at construction it went on attesting to a signing key `rotateSignKey` had replaced,
   *  so a deployment that booted strict-clean and rotated to a five-byte secret still reported clean. */
  get __weakSigningKey(): boolean {
    // RFC 7518 section 3.2 puts a floor under the HMAC algorithms; an asymmetric `key` is a PEM, whose
    // strength is in the key it encodes and not in its length. Reported, not thrown: a short secret is a
    // production footgun, and `strict({ env: 'production' })` is where those are refused.
    return (this._signKey.alg ?? 'HS256').startsWith('HS') && Buffer.byteLength(this._signKey.key, 'utf8') < 32
  }

  constructor(private readonly _cfg: JwtTransport.Cfg) {
    assertKeyMaterial(_cfg.signKey, 'signKey')
    ...
    this._refreshSameSite = _cfg.refresh?.sameSite ?? 'strict'
   ...
   *  host's own refresh endpoint to read. Caps at 4KB and refuses a comma-joined header. */
    ...
    if (this._refreshEnabled) {
      ...
      intents.unshift({
        ...
        options: {
          httpOnly: true,
          secure: true,
          sameSite: this._refreshSameSite,
          path: '/',
          maxAge: expiresInSec,
        },
      })
    ...
    return [
      {
        type: 'clearCookie',
        name: this._refreshCookieName,
        // Same attributes as the set, or the browser keeps the cookie the clear was meant to remove.
        options: { httpOnly: true, secure: true, sameSite: this._refreshSameSite, path: '/', maxAge: 0 },
      },
    ]
  }
  ...
  verify(token: string): Answer.Me<Sessions.Me> {
    return answer(async () => {
      ...
      if (rest.length > 0 || headerB64 === undefined || payloadB64 === undefined || sig === undefined) {
        throw new AuthError('AUTH_JWT_INVALID', { reason: 'token is not a three-part JWS' })
      }
      ...
      try {
        ...
        throw new AuthError('AUTH_JWT_INVALID', { reason: 'the header is not decodable base64url JSON' })
      }
      const header = parseJwtHeader(rawHeader)
      if (!header)
        throw new AuthError('AUTH_JWT_INVALID', { reason: 'the header is missing or has a malformed alg or kid' })

      const key = this._verifyKeys.get(header.kid)
      if (!key) throw new AuthError('AUTH_JWT_KEY_UNKNOWN', { kid: header.kid })
      ...
      if (header.alg !== expectedAlg)
        throw new AuthError('AUTH_JWT_INVALID', {
      ...
      if (isExpiredAt(key.notAfter))
        throw new AuthError('AUTH_JWT_INVALID', { reason: 'the verification key is past its notAfter' })

      if (!jwsVerify(expectedAlg, key.key, `${headerB64}.${payloadB64}`, sig))
        throw new AuthError('AUTH_JWT_INVALID', { reason: 'the signature does not verify under that key' })

      let rawPayload: unknown
      try {
        rawPayload = JSON.parse(base64urlDecode(payloadB64))
      } catch {
        throw new AuthError('AUTH_JWT_INVALID', { reason: 'the payload is not decodable base64url JSON' })
      }
      const payload = parseJwtPayload(rawPayload)
      if (!payload)
        throw new AuthError('AUTH_JWT_INVALID', { reason: 'the payload is missing or has a malformed claim' })

      const nowSec = Math.floor(Date.now() / 1000)
      const skew = this._clockSkewSec
      if (payload.exp < nowSec - skew) throw new AuthError('AUTH_SESSION_EXPIRED', { expiredAt: payload.exp * 1000 })
      // The ceiling `exp` cannot outlive.
      if (payload.abs !== undefined && payload.abs < nowSec - skew)
        throw new AuthError('AUTH_SESSION_EXPIRED', { expiredAt: payload.abs * 1000 })
      // Every timestamp on the reconstructed session below is derived from `iat`, so a future one dates
      // the whole session forward.
      if (payload.iat > nowSec + JWT_FORWARD_SKEW_SEC + skew)
        throw new AuthError('AUTH_JWT_INVALID', { reason: 'iat is further ahead than the clock-skew allowance' })
      if (payload.iss !== this._cfg.issuer)
        throw new AuthError('AUTH_JWT_INVALID', { reason: 'iss does not match the configured issuer' })
      if (this._cfg.audience !== undefined && payload.aud !== this._cfg.audience)
        throw new AuthError('AUTH_JWT_INVALID', { reason: 'aud does not match the configured audience' })
      ...
      const session: Sessions.Me = {
        ...
        // Not `exp`: that is the access token's own minutes, this is the session's ceiling.
        absoluteExpiresAt: new Date((payload.abs ?? payload.exp) * 1000),
      ...
      if (session.actingAs?.expiresAt !== undefined && isExpiredAt(session.actingAs.expiresAt)) {
        throw new AuthError('AUTH_IMPERSONATE_WINDOW_CLOSED', {
          closedAt: session.actingAs.expiresAt.getTime(),
        })
      }
      return session
    })
  ...
  rotateSignKey(opts: JwtTransport.IRotateOpts): void {
    ...
    assertKeyMaterial(newSign, 'signKey')
    if (opts.verifyKey) assertKeyMaterial(opts.verifyKey, 'verifyKey')
