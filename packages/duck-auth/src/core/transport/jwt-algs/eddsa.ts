...
export function authSignEddsa(key: string, signingInput: string): string {
  const pk = createPrivateKey(key)
  return ed25519Sign(null, Buffer.from(signingInput), pk).toString('base64url')
}

export function authVerifyEddsa(key: string, signingInput: string, sigB64: string): boolean {
  try {
