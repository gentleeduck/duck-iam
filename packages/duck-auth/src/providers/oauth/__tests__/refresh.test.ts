...
import { OAuthRefresh, projectAccessToken, refreshOauthToken } from '../core/refresh'
...
describe('OAuth refresh-token reuse detection (RFC 6749 section 10.4)', () => {
    ...
    await adapter.credentials.upsert(
      {
        ...
        metadata: {
          ...
          generation: 1,
          accessToken: 'at-1',
        } satisfies OAuthRefresh.IFamilyMetadata,
      },
      {},
    )
  }
  ...
  it('happy path rotates the refresh token + bumps generation', async () => {
    ...
    const exchange = vi.fn(
      async (): Promise<OAuthClient.ITokenResponse> => ({
        access_token: 'at-2',
    ...
    expect((newRow?.metadata as OAuthRefresh.IFamilyMetadata).generation).toBe(2)
  ...
  it('IdP that does not rotate (no refresh_token in response) keeps the current row + updates access token', async () => {
    ...
    expect((row?.metadata as OAuthRefresh.IFamilyMetadata).accessToken).toBe('at-new')
  })

  it('concurrent refreshes with the same token - only one wins, loser revokes family', async () => {
    await seedRefresh('rt-old')
    let exchangeCalls = 0
    const slowExchange = async (): Promise<OAuthClient.ITokenResponse> => {
      exchangeCalls++
  ...
  describe('family metadata validation', () => {
    ...
    it('refreshOauthToken throws PROVIDER_FAILED on malformed family metadata', async () => {
