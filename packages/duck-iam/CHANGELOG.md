...
- 919e62e: Security and correctness fixes from the ongoing audit, grouped by what goes wrong
  ...
  **Your denies could go missing while your grants survived.** Over HTTP, a list
  ...
  and only the explicit denies vanished. A single unreadable _row_ already refused
  ...
  **A grant you issued could silently not exist.** In the drizzle adapter an
  ...
  elapsed target deleted the _live_ source row and returned `true`. A live
  ...
  **A corrupt role row could turn a deny into an allow.** Every adapter refused a
  malformed _policy_ row but skipped a malformed _role_ row, on the grounds that a
