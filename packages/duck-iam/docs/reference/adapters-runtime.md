...
| `updateAssignmentScope` | `(subjectId, roleId, fromScope, toScope, actor?) => Promise<boolean>` | optional | Moves one grant in place. `false` when there is no such grant — and `false` must mean *nothing was written*. Both ends go through `iamAssertAssignableScope`: the `from` as a lookup, the `to` as a grant. |
