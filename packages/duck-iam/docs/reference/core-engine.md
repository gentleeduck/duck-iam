...
defines — dropping it would let a deleted role's id be re-added by a phantom —
but it cannot walk what that id inherits, because there is no row to read
`inherits` from. So the subject keeps a role that matches an ABAC rule naming it
while quietly holding none of the roles it conferred, and a deny targeting one
of *those* stops applying. Round 58's check does not see this: the role the
policy targets is stored and perfectly healthy; it is the intermediate row that
is missing. Every adapter that drops a malformed role row rather than refusing
it — prisma, http, file, drizzle — can produce exactly this state, which is what
makes the drop tolerable: the grant left pointing at nothing is reported here.
It is de-duplicated per role id rather than per `(subject, role)` pair, since
