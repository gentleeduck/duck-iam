...
```typescript
import type { InferAction, InferResource, InferScope } from 'duck-iam'
...
```typescript
import type { Adapter, Attributes, Policy, Role, ScopedRole } from 'duck-iam'
...
<MermaidDiagram chart={`flowchart TD
    subgraph Caches["Engine Caches"]
        direction TB
        PC["policyCache (size: 1)\\nKey: 'all' -> Policy[]"]
        RC["roleCache (size: 1)\\nKey: 'all' -> Role[]"]
        RBC["rbacPolicyCache (size: 1)\\nKey: 'rbac' -> Policy"]
        SC["subjectCache (size: maxCacheSize)\\nKey: subjectId -> Subject"]
    end
...
<Steps>
  ...
  <Step>
    **Test server middleware (Express example)**
    ...
    import { accessMiddleware, guard } from 'duck-iam/integrations/express'
...
<details>
<summary>Full production `src/access.ts`</summary>
