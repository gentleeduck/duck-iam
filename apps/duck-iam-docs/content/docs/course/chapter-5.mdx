  ...
  <Step>
    **Set up scoped assignments**
    ...
    const adapter = new MemoryAdapter({
      ...
      scopedAssignments: {
        'alice': {
          'acme': ['admin'],       // alice is admin in acme
          'globex': ['viewer'],    // alice is viewer in globex
        },
        'bob': {
          'acme': ['editor'],
          'globex': ['editor'],
    ...
    Base roles apply everywhere. Scoped roles are added on top when a scope is provided.
  </Step>
...
<MermaidDiagram chart={`flowchart LR
    subgraph Resolution["Role Resolution with Scope"]
        direction TB
        S1["1. Get base roles\\nalice -> ['viewer']"]
        S2["2. Get scoped roles\\nalice + acme -> ['admin']"]
        S3["3. Merge + deduplicate\\n['viewer', 'admin']"]
        S4["4. Resolve inheritance\\nviewer + editor + admin"]
        S5["5. Build __rbac__ policy\\nwith all permissions"]
    end
