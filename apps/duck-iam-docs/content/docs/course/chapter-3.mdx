...
<Accordion type="multiple" collapsible className="w-full">
  ...
  <AccordionItem value="c3-targets">
    ...
    <AccordionContent className="text-muted-foreground">
      ...
      every request. With targets, the policy's rules are not evaluated when the target does not
      match, reducing evaluation time. One detail: `targets.resources` uses direct resource
