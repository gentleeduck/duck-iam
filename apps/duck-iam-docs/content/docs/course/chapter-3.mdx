...
<Steps>
  <Step>
    **Create an owner-only policy**
    ...
    - `.algorithm('deny-overrides')` -- any deny wins
    - `.deny()` -- this rule's effect is deny
    - `.on('update', 'delete')` -- applies to these actions
    ...
    - `.when(...)` -- condition that must be true for the rule to fire
    - `'resource.attributes.ownerId'` -- reads ownerId from the resource
  ...
  <Step>
    **Test with resource attributes**
    ...
    `ownerId` in the resource attributes is compared to `$subject.id` (resolves to `'bob'`).
    When they differ, the deny rule fires.
  </Step>
</Steps>
...
```typescript
// All other methods are shortcuts for this
...
<Accordion type="multiple" collapsible className="w-full">
  <AccordionItem value="c3-rbac-abac-order">
    ...
    <AccordionContent className="text-muted-foreground">
      Roles are converted to a synthetic `__rbac__` policy evaluated first, followed by
      your custom policies. They are AND-combined: all must allow. The engine
  ...
  <AccordionItem value="c3-no-attrs">
    ...
    <AccordionContent className="text-muted-foreground">
      ...
      `null neq 'bob'` as `true`, so the deny rule fires. Missing attributes always deny.
    </AccordionContent>
  </AccordionItem>
  ...
  <AccordionItem value="c3-admin-exempt">
    ...
    <AccordionContent className="text-muted-foreground">
      Add `.not(n => n.role('admin'))` to the condition. The rule becomes: "deny if
      not the owner AND not an admin." This is what the checkpoint code shows.
    </AccordionContent>
  </AccordionItem>
  ...
  <AccordionItem value="c3-multiple-policies">
    ...
    <AccordionContent className="text-muted-foreground">
      ...
      All must allow for the overall result to be allow.
    </AccordionContent>
  </AccordionItem>
  ...
  <AccordionItem value="c3-when-vs-whenAny">
    ...
    <AccordionContent className="text-muted-foreground">
      `.when()` is AND -- all conditions must pass. `.whenAny()` is OR -- any condition
      is sufficient. For complex logic, nest `.and()`, `.or()`, and `.not()` inside
      `.when()`. The most common pattern is `.when()` with an inner `.not()` for exemptions.
    </AccordionContent>
  </AccordionItem>
  ...
  <AccordionItem value="c3-targets">
    ...
    <AccordionContent className="text-muted-foreground">
      ...
      rule is evaluated on every request. Note: `targets.resources` uses direct resource
      matching, not the hierarchical dot-aware matcher that normal rule resources use.
    </AccordionContent>
  </AccordionItem>
  ...
  <AccordionItem value="c3-forScope">
    ...
    <AccordionContent className="text-muted-foreground">
      `.forScope('acme')` adds a scope condition so the rule only fires when the request
      scope is `acme`. Multiple scopes: `.forScope('acme', 'globex')`. The scope condition
      is merged with `.when()` conditions using AND.
    </AccordionContent>
  </AccordionItem>
