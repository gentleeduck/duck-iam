...
<Accordion type="multiple" collapsible className="w-full">
  ...
  <AccordionItem value="c7-refresh">
    ...
    <AccordionContent className="text-muted-foreground">
      ...
      `access.update(newMap)` -- subscribers are notified automatically. In Vue, call
  ...
  <AccordionItem value="c7-performance">
    ...
    <AccordionContent className="text-muted-foreground">
      Yes. `can()` is a synchronous O(1) key lookup -- no network calls, no async operations,
  ...
  <AccordionItem value="c7-usePerms-vs-provider">
    ...
    <AccordionContent className="text-muted-foreground">
      Use `AccessProvider` with server-rendered permissions (SSR/RSC) -- no loading state,
