...
description: Engine methods, hooks, caching, batch permissions, explain/debug, and the Admin API.
---
...
<Steps>
  <Step>
    **beforeEvaluate -- enrich the request**
    ...
    The hook receives the full `AccessRequest` and returns the (possibly modified) request.
    Any field is fair game: subject attributes, resource attributes, environment.

    If `beforeEvaluate` throws, evaluation is skipped, the result is deny (fail closed), and
    `onError` is called.
  </Step>
...
To check multiple permissions in one call, use `engine.permissions()`:
...
Use `engine.explain()` to trace why a permission was allowed or denied:
...
Every mutation automatically invalidates the relevant cache — no manual invalidation needed after admin operations.
...
<Accordion type="multiple" collapsible className="w-full">
  ...
  <AccordionItem value="c4-permissions-scoped">
    ...
    <AccordionContent className="text-muted-foreground">
      Yes. Each check can include an optional `scope` field:
