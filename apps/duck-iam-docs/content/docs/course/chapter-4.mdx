...
Use `authorize()` directly when you already have a fully resolved subject object
and want to skip the adapter lookup.

## Dev vs Prod Mode

The engine supports two modes that control what methods like `check()` and `permissions()`
return. By default, the engine runs in **development** mode with rich `Decision` objects,
debug traces, and the `explain()` method. In **production** mode, the engine strips that
overhead and returns fast booleans instead.
...
`can()` always returns a plain `boolean` regardless of mode. It is the simplest API and
behaves identically in both modes.

`check()` is mode-dependent. Its return type is `ModeResult<TMode>` -- in development
mode that resolves to `Decision`, in production mode it resolves to `boolean`. This lets
you use `check()` during development to inspect reasons and timing, then switch to
production mode for zero-overhead boolean checks without changing any call sites.

`explain()` is only available in development mode. It evaluates all rules without
short-circuiting and returns a full debug trace. In production mode, calling `explain()`
would be a type error.

### Development Mode

Development mode is the default. Use it during development, testing, and debugging:
...
Production mode strips debug information and returns fast booleans. Use it in deployed
applications where you only need yes/no answers:
...
the evaluation trace). That keeps explain free of those side-effect hooks, but it is not
fully fail-closed: if subject resolution, `beforeEvaluate`, or policy loading throws, the
`explain()` call rejects.

## Caching

The engine maintains four LRU (Least Recently Used) caches to avoid hitting the adapter
on every check:
...
4. **Subject cache** (up to `maxCacheSize` entries) -- stores per-user data: resolved roles,
   scoped roles, and attributes. Uses LRU eviction when full -- the least recently accessed
   subject is dropped when a new one is added.
...
This lets you see exactly which condition failed, what value was expected, and what value
was actually present. Invaluable for debugging complex policies.
...
<Accordion type="multiple" collapsible className="w-full">
  ...
  <AccordionItem value="c4-explain-production">
    ...
    <AccordionContent className="text-muted-foreground">
      `explain()` is only available in development mode. If your engine is created with
      `mode: 'production'`, `explain()` is not available. To use it in a deployed environment,
  ...
  <AccordionItem value="c4-authorize-vs-can">
    ...
    <AccordionContent className="text-muted-foreground">
      ...
      your own data source) and want to skip the adapter lookup. This is useful in
      middleware where you resolve the user once and check multiple permissions, or
      in tests where you construct subjects manually.
    </AccordionContent>
  </AccordionItem>
