...
When you call `can()` or `check()`, they call `resolveSubject()` to load the subject
...
The result is cached. Subsequent calls for the same subject return the cached version
until the cache TTL expires or you manually invalidate.
