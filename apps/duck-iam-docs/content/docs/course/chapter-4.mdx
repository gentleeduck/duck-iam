...
```typescript
import type { AccessRequest } from 'duck-iam'
...
<MermaidDiagram chart={`flowchart LR
    subgraph Caches["4 LRU Caches"]
        direction TB
        PC["Policy Cache\\nall policies from adapter\\n(1 entry)"]
        RC["Role Cache\\nall role definitions\\n(1 entry)"]
        RB["RBAC Policy Cache\\nsynthetic __rbac__ policy\\n(1 entry)"]
        SC["Subject Cache\\nper-user roles + attributes\\n(up to maxCacheSize entries)"]
    end
...
<details>
<summary>Full engine with all features</summary>

```typescript
import { Engine, validateRoles } from 'duck-iam'
