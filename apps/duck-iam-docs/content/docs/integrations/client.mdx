...
For code outside React components (utilities, event handlers, tests), use `createIamPermissionChecker`. It takes a `PermissionMap` and returns a checker object with `can`, `cannot`, and the raw `permissions`.
...
`fromServer` sends a GET request, parses the JSON response as a `PermissionMap`, and returns a new `IamAccessClient`. It throws if the response is not OK (non-2xx status).
...
| --- | --- | --- |
| `constructor(perms?)` | `new (PermissionMap?) -> IamAccessClient` | Create from a permission map |
