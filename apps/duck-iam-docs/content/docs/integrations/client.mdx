...
You usually do not need to call `buildPermissionKey()` yourself because the React, Vue,
and vanilla clients do it internally. It is useful when you generate or inspect
`PermissionMap` objects manually in tests, server endpoints, or custom client helpers.
...
These helpers are best for UI summarization such as navigation menus, toolbar groups, or
"does this user have anything on posts at all?" checks. They scan the existing
`PermissionMap`; they do not replace a fresh instance-level permission evaluation on the server.
...
<Accordion type="multiple" collapsible className="w-full">
  ...
  <AccordionItem value="vue-useaccess-throws">
    ...
    <AccordionContent className="text-muted-foreground">
      The Vue integration assumes permission state must be provided explicitly. Throwing early helps you catch missing
      provider or plugin setup immediately instead of silently rendering a locked-down UI with no explanation.
    </AccordionContent>
  </AccordionItem>
