...
Use the generic helpers when the built-in framework wrappers are too opinionated, or when
you want the same access-control glue across several runtimes.
...
- `createSubjectCan(engine, subjectId, environment?)` returns a subject-bound
  `can(action, resource, resourceId?, scope?)` function that is convenient in handlers,
  jobs, and service objects.
...
<Accordion type="multiple" collapsible className="w-full">
  ...
  <AccordionItem value="middleware-plus-handler-checks">
    ...
    <AccordionContent className="text-muted-foreground">
      Yes. That is the recommended split. Middleware and route wrappers are for coarse-grained gating, while handler-level
      checks are where you should enforce owner checks, attribute-based restrictions, and other resource-instance rules.
    </AccordionContent>
  </AccordionItem>
  ...
  <AccordionItem value="middleware-owner-checks">
    ...
    <AccordionContent className="text-muted-foreground">
      Because ownership usually depends on resource attributes that are not in the raw route shape. Middleware can often
      tell you “this is a delete on post 42,” but it cannot know `ownerId`, status, or tenant metadata until your handler
      loads the actual record. That is why record-level checks still belong near the data fetch.
    </AccordionContent>
  </AccordionItem>
  ...
  <AccordionItem value="next-pattern-prefix">
    ...
    <AccordionContent className="text-muted-foreground">
      No. String patterns are checked with simple path-prefix matching. If you need stricter matching,
      optional segments, or glob-like behavior, use a <code className="rounded bg-muted px-2 py-1">RegExp</code>
