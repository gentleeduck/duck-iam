...
- `iamExtractEnvironment(req)` builds the default `{ ip, userAgent, timestamp }` environment
...
import { iamActionForMethod, iamDefaultResource } from '@gentleduck/iam/server/generic'
...
and a rule written against `admin` never fires. `iamDefaultResource` normalises first:
decode once, collapse repeated slashes, resolve dot segments.
...
|---|---|---|
| `iamActionForMethod` | `(method: string \| undefined) => string` | Upper-cases before the lookup, so a lowercase `delete` cannot miss the map. Never returns `undefined`. |
| `IAM_UNKNOWN_ACTION` | `'unknown'` | What `iamActionForMethod` returns for an unrecognised or missing method. |
| `iamNormalizePathname` | `(pathname: string) => string` | Decodes once, collapses `//` runs, then resolves `.`/`..` — in that order, because decoding can reveal new dot segments. A malformed escape keeps the raw path rather than throwing. |
| `iamDefaultResource` | `(pathname: string \| undefined) => { type, id, attributes }` | Normalises first, then derives `{ type, id }`. `attributes` is always present (`{}`) because `IResource` requires it. |
...
The `iamGuard` function catches errors and passes them to Express's `next(err)`, so they
reach your error-handling middleware. The `iamAccessMiddleware` catches errors and calls
the `onError` option.
...
Handlers without an `@IamAuthorize` decorator are allowed by default -- the guard only activates when metadata is present.
...
```
import { withIamAccess, checkIamAccess, getIamPermissions, createIamNextMiddleware } from '@gentleduck/iam/server/next'
```
