...
`'unknown'`. That is deliberate: `'unknown'` matches no policy target, so a
request the helpers could not classify is **denied**, not authorized against
whatever the raw method or path happened to spell.

The value they replace is `undefined` — and `undefined` matches no *deny* rule
either, which is the opposite outcome. That asymmetry is the entire reason these
exist. Do not "improve" them by falling back to `'read'` or to the raw path
segment; both turn an unclassifiable request into an allowed one.
</Callout>
