...
interface CustomAdapter<TAction extends string, TResource extends string, TRole extends string, TScope extends string>
  ...
  revokeRoleMany?(rows: readonly IamAdapter.ITripleRow<TRole, TScope>[]): Promise<readonly number[] | null>
...
`IAssignOptions` carries the per-grant extras `assignRole` accepts:
`startsAt`, `expiresAt` and `attributes`.
