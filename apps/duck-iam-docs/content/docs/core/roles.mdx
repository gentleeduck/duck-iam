...
You can scope individual permissions within a role by passing an optional third
argument to `grant()`:
...
The `grantScoped(scope, action, resource)` method is also available as an
alternative with scope as the first argument.
...
typed builder functions. When you declare `roles`, the role ID parameter in `defineRole()`
and the `.role()` / `.roles()` methods in conditions are constrained to your declared values.
