...
<Callout type="info">
Value parameters use the `ConditionValue` type which splits the accepted value into its
string and non-string parts. Non-string types (like `number` for `env('hour', ...)`) pass
through unchanged, so passing a string where a number is expected is a compile error.
String types get `DollarPaths<TContext>` added for `$`-reference autocomplete. When the
field resolves to broad `string`, an `(string & {})` trick preserves the `$`-prefixed
literals in autocomplete suggestions.
</Callout>
