...
| Type | Purpose |
|---|---|
| `DotPaths<T>` | Generates all valid dot-separated paths through `T` (e.g. `'subject.attributes.status'`). Arrays are treated as leaf paths and functions are skipped, so only real data properties appear. |
...
Field-path autocomplete and value autocomplete are related, but they are not the same thing.

- If a field resolves to a narrow literal union such as `'draft' | 'published'`, the value
  side can stay narrow and still offer `$subject.*` / `$resource.*` / `$environment.*`
  references.
...
For the best autocomplete, make the parts of your context that matter most explicit:
...
That gives you better value narrowing in `.check()`, `.eq()`, `.neq()`, `.attr()`,
...
<Accordion type="multiple" collapsible className="w-full">
  ...
  <AccordionItem value="value-autocomplete-broad">
    ...
    <AccordionContent className="text-muted-foreground">
      Because value typing comes from the resolved field type. If your context says a field is just
