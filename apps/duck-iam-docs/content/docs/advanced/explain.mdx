...
<Callout type="warning" title="Development mode only">
  `engine.explain()` is only available when the engine is created with `mode: 'development'` (the default).
  Calling `explain()` on a production-mode engine throws an error at runtime, and TypeScript will
  flag the call at compile time. Use development mode for debugging and production mode for speed.
</Callout>
...
`explain()` is a diagnostic tool. It applies the `beforeEvaluate` hook so the traced request
matches the real evaluation input, but it does **not** trigger `afterEvaluate`, `onDeny`,
or `onError`. One caveat matters: if `beforeEvaluate` throws, the explain call rejects because
that hook still runs on the main explain path.

```typescript
// Engine must be in development mode (the default)
const engine = createEngine({ mode: 'development', /* ... */ })
...
When `targetMatch` is `false`, the policy's rules were not evaluated because its target filter
(actions, resources, or roles) did not match the request. The `rules` array will be empty. In
the current trace format, `result` still mirrors the engine's default effect, so use
`targetMatch` as the signal that the policy was bypassed.
...
<Accordion type="multiple" collapsible className="w-full">
  ...
  <AccordionItem value="explain-prod-usage">
    ...
    <AccordionContent className="text-muted-foreground">
      No. Calling <code className="rounded bg-muted px-2 py-1">explain()</code> on a production-mode engine throws
      <em>"explain() is not available in production mode. Use mode: 'development' to enable debug tools."</em>{" "}
      TypeScript also prevents the call at compile time because <code className="rounded bg-muted px-2 py-1">explain()</code>
      requires <code className="rounded bg-muted px-2 py-1">Engine&lt;..., 'development'&gt;</code>. Use development mode
      for debugging and investigation; use production mode for speed and a smaller API surface.
    </AccordionContent>
  </AccordionItem>
</Accordion>
