...
| `mode` | `'development' \| 'production'` | `'development'` | Controls return types and feature availability. See [Development vs Production Mode](#development-vs-production-mode). |
...
const engine = new Engine({
  adapter,
  mode: 'development', // 'production' for boolean-only fast path
  defaultEffect: 'deny',
  cacheTTL: 120,
  maxCacheSize: 5000,
  hooks: {
...
The simplest check. Always returns `boolean` regardless of engine mode.
...
Same as `can()` but returns richer data. The return type depends on the engine mode via `ModeResult<TMode>`:

- **Development mode** (default): returns a full `Decision` object with metadata.
- **Production mode**: returns a `boolean` for maximum speed.
...
const allowed = await engine.check('user-1', 'delete', {
  type: 'post',
  id: 'post-123',
  attributes: { ownerId: 'user-2' },
})
...
`check()` call internally. Use it when you already have a resolved subject. Returns
`ModeResult<TMode>` -- a `Decision` in development mode, a `boolean` in production mode.
...
Batch check multiple permissions at once. Loads data once, evaluates many. Returns
`ModePermissionMap<TMode>` -- a `PermissionMap` (values are `Decision`) in development mode,
or a `Record<string, boolean>` in production mode. Keys use one of four forms:
...
Returns a full evaluation trace. **Only available in development mode.** Calling `explain()`
on a production-mode engine throws an error. See the [Explain and Debug](/docs/advanced/explain)
...
In **production mode**, hooks other than `beforeEvaluate` are skipped entirely for maximum
throughput. If you need auditing or alerting in production, handle it outside the engine
(e.g., structured logging in your request handler).
...
The `mode` option on `EngineConfig` controls the trade-off between debuggability and raw speed.

| | Development (default) | Production |
| --- | --- | --- |
| `engine.can()` | `boolean` | `boolean` |
| `engine.check()` | `Decision` | `boolean` |
| `engine.authorize()` | `Decision` | `boolean` |
| `engine.permissions()` | `PermissionMap` (values are `Decision`) | `Record<string, boolean>` |
| `engine.explain()` | Full trace | Throws |
| Hooks (`afterEvaluate`, `onDeny`, `onError`) | Run normally | Skipped |

Development mode is the default. It gives you `Decision` objects with timing, matched rules,
and human-readable reasons -- everything you need while building and debugging policies. Hooks
run on every evaluation so you can log, audit, and alert.

Production mode strips all of that away. Methods return plain booleans, hooks other than
`beforeEvaluate` are skipped, and `explain()` is disabled. The result is a smaller, faster
evaluation path.

```typescript
// Development -- full diagnostics
const devEngine = new Engine({ adapter }) // mode defaults to 'development'

// Production -- booleans only, no hooks overhead
const prodEngine = new Engine({ adapter, mode: 'production' })
```

The generic `Mode` type flows through the engine so TypeScript narrows return types
automatically. The key utility types are:

- `Mode` -- `'development' | 'production'`
- `ModeResult<M>` -- `Decision` when `M` is `'development'`, `boolean` when `M` is `'production'`
- `ModePermissionMap<M>` -- `PermissionMap` or `Record<string, boolean>` accordingly

## Pure Evaluation Functions

When you need to evaluate policies without an engine instance -- for example inside a
serverless edge function or a Web Worker -- two pure functions are available:

```typescript
import { evaluateFast, evaluatePolicyFast } from '@gentleduck/iam'
```

### evaluateFast

Evaluates an array of policies against a request and returns a `boolean`. No caching, no
hooks, no adapter -- just the core matching logic.

```typescript
import { evaluateFast } from '@gentleduck/iam'

const allowed = evaluateFast(policies, {
...
Same idea, but evaluates a single policy instead of an array.

```typescript
import { evaluatePolicyFast } from '@gentleduck/iam'

const allowed = evaluatePolicyFast(officeHoursPolicy, {
  subject: { id: 'user-1', roles: ['editor'], attributes: {} },
  action: 'update',
  resource: { type: 'post', attributes: {} },
  environment: { hour: 14 },
...
Both functions are stateless and synchronous. They are a good fit for hot paths where you
already have the policies in memory and want to avoid the async overhead of the engine.
