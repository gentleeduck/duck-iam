...
<Accordion type="multiple" collapsible className="w-full">
  ...
  <AccordionItem value="runtime-vs-monorepo-requirements">
    ...
    <AccordionContent className="text-muted-foreground">
      The published package targets application runtimes, while the monorepo also includes docs, examples,
      and workspace tooling. In practice, duck-iam itself is designed for Node.js 18+ runtimes, while the
      monorepo may require newer Node or Bun versions to run the entire workspace.
    </AccordionContent>
  </AccordionItem>
  ...
  <AccordionItem value="need-create-access-config">
    ...
    <AccordionContent className="text-muted-foreground">
      No. You can start with the untyped builders and move to <code className="rounded bg-muted px-2 py-1">createAccessConfig()</code>
      when you want compile-time validation for actions, resources, scopes, and roles. The typed path is the
      better long-term default for production apps, but it is not required to get started.
    </AccordionContent>
  </AccordionItem>
  ...
  <AccordionItem value="what-to-seed-first">
    ...
    <AccordionContent className="text-muted-foreground">
      ...
      needed to boot the app. Subject attributes can usually be added later as your domain data becomes
      available.
    </AccordionContent>
  </AccordionItem>
  ...
  <AccordionItem value="mix-root-and-subpaths">
    ...
    <AccordionContent className="text-muted-foreground">
      Yes. That is the normal setup. Import builders, types, and the engine from
  ...
  <AccordionItem value="memory-root-vs-subpath">
    ...
    <AccordionContent className="text-muted-foreground">
      It is exported from the root as a convenience because it is the default starting adapter for tests, demos, and
      quick prototypes. The dedicated subpath still exists for consistency with the other adapters.
    </AccordionContent>
  </AccordionItem>
