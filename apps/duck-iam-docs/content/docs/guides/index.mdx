...
By the end, you will have a complete authorization system that spans from your database
layer to your UI components.

## Step 1: Define Your Access Config

Start by declaring the actions, resources, and scopes your application uses. This creates
a typed factory that constrains all subsequent builders:
...
The `as const` assertion is critical -- it tells TypeScript to infer literal types rather
than `string[]`. After this, calling `access.defineRole("x").grant("craete", "post")` will
produce a compile error because `"craete"` is not in the actions list.
...
The inheritance chain works as follows: `admin` inherits from `moderator`, which inherits
from `editor`, which inherits from `viewer`. An admin automatically has every permission
from all three parent roles, plus its own direct grants.
...
import { MemoryAdapter } from "duck-iam";
import { access } from "./access";
...
RBAC handles the common cases. For more granular control, add ABAC policies. Here is
...
Now write operations are denied outside business hours, while reads remain allowed at
any time. The `first-match` algorithm evaluates rules in order: the deny rule fires first
for writes outside 9-18 UTC, and the catch-all allow rule handles everything else.

## Step 5: Owner-Only Conditions

A common pattern is allowing users to edit only resources they own. Use the `$subject.id`
variable reference to compare the requesting user against the resource owner:
...
The `isOwner()` helper generates a condition that checks
`resource.attributes.ownerId eq $subject.id`. At evaluation time, `$subject.id` is resolved
to the actual subject ID from the request.

## Step 6: Multi-Tenant Scoped Roles

In multi-tenant applications, users often have different roles in different organizations.
duck-iam supports this with scoped role assignments:
...
The engine resolves scoped roles by matching the request scope against the user's scoped
role assignments. Only roles assigned to the matching scope (plus any global roles) are
...
import { withAccess } from "duck-iam/server/next";
import { engine } from "@/lib/engine";
...
The `AccessProvider` receives the `PermissionMap` generated on the server (from Step 7).
Permission checks on the client are synchronous lookups -- no network requests, no latency.

## Debugging with explain()

When a permission check produces unexpected results, use `engine.explain()` to get a full
...
This is invaluable during development and when troubleshooting production authorization
issues.
